...
for pool_size in pool_sizes:
  ...
  maxpooling = MaxPooling2D(pool_size=(pool_size,pool_size), padding='same')(input)

  model = Model(inputs=[input], outputs=[maxpooling])
  ...
  model_name = "maxpooling2d_" + str(pool_size)
  ...
  if not os.path.isdir("vpu_models/maxpooling2d_" + str(pool_size) + "/"):
    os.mkdir("vpu_models/maxpooling2d_" + str(pool_size) + "/")

  model.save("vpu_models/maxpooling2d_" + str(pool_size) + "/saved_model/")

  # save in frozen graph format #
  imported = tf.saved_model.load("vpu_models/maxpooling2d_" + str(pool_size) + "/saved_model/")
  ...
  tf.io.write_graph(graph_def, "vpu_models/maxpooling2d_" + str(pool_size) + "/", 'frozen_graph.pb', as_text=False)
