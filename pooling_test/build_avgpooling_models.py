...
from tensorflow.keras.layers import Input, Conv2D, Flatten, AveragePooling2D
from tensorflow.keras.models import Model
from tensorflow.python.framework.convert_to_constants import convert_variables_to_constants_v2

pool_sizes = [2,4,6,8,10,12,14,16]
input_size = 32
...
for pool_size in pool_sizes:
  ...
  pooling = AveragePooling2D(pool_size=(pool_size,pool_size), padding='same')(input)

  model = Model(inputs=[input], outputs=[pooling])
  ...
  model_name = "avgpooling2d_" + str(pool_size)
  ...
  if not os.path.isdir("vpu_models/avgpooling2d_" + str(pool_size) + "/"):
    os.mkdir("vpu_models/avgpooling2d_" + str(pool_size) + "/")

  model.save("vpu_models/avgpooling2d_" + str(pool_size) + "/saved_model/")

  # save in frozen graph format #
  imported = tf.saved_model.load("vpu_models/avgpooling2d_" + str(pool_size) + "/saved_model/")
  ...
  tf.io.write_graph(graph_def, "vpu_models/avgpooling2d_" + str(pool_size) + "/", 'frozen_graph.pb', as_text=False)
