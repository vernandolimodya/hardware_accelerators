...
class evalSession():
    ...
    def __init__(self, model_name, session_name="evalSession", generated_inputs=1e4):
        ...
        self.input_size = 32
        self.model_float_ops = 1 # i don't know how "finding the maximum" are calculated in terms of ops
    ...
    def experiment(self, batch_sizes, reps=1, verbose=False):
        ...
        for i in range(len(batch_sizes)):
            ...
            for _ in range(reps):
                ...
                cpu = {'model_load_time' : 1, 'results_arr': 1, 'latency': 1, 'fps': 1, 'flops': 1}
    ...
    def plot_results(self, batch_sizes, reps=1, verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_vpu_power/"):
            os.mkdir("output_files_vpu_power/")

        if not os.path.exists("output_files_vpu_power/" + filename + "/"):
            os.mkdir("output_files_vpu_power/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_vpu_power/" + filename + "/" + filename + ".csv")
...
if __name__=="__main__":
    ...
    for i in [2,4,6,8,10,12,14,16]:
        model_names.append("maxpooling2d_" + str(i) )
