...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            interpreter = tflite.Interpreter(model_path='tpu_models/optimized_model.tflite')
        elif device == 'TPU':
            interpreter = tflite.Interpreter(model_path='tpu_models/optimized_model_quantized_edgetpu.tflite', 
...
if __name__=="__main__":
    ...
    ex_fp32 = evalSession(tflite_model_path=" ", input_file_folder="inputs/numpy_fp32", model_float_ops=651083000, generated_inputs=10000)
    ex_fp32.plot_results(batch_sizes, reps=1, pars=['latency', 'fps', 'accuracy_degr'], verbose=True)
    del ex_fp32
