...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            self.net = self.ie_core.read_network(model="vpu_models/optimized_model/ir_model.xml", weights="vpu_models/optimized_model/ir_model.bin")
        elif device == 'MYRIAD':
            self.net = self.ie_core.read_network(model="vpu_models/optimized_model/ir_model_compressed.xml", weights="vpu_models/optimized_model/ir_model_compressed.bin")
...
if __name__=="__main__":
    ...
    ex_fp32.export_results(batch_sizes, reps=1, pars=['latency', 'fps', 'flops', 'accuracy_degr'], verbose=True)
    del ex_fp32