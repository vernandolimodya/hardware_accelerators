...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        input_data = np.array([np.load("inputs/numpy_fp32_concat/convertedInput_" + str(i) + ".npy").reshape((self.input_size,self.input_size,1)) for i in range(self.generated_inputs)])
    ...
    def plot_results(self, batch_sizes, reps=1, verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_tpu/"):
            os.mkdir("output_files_tpu/")
...
if __name__=="__main__":
    ...
    small_models = ["MobileNet"]

    # large_models = ["VGG16", "VGG19", "ResNet50", "DenseNet121"]

    large_models = ["VGG16", "VGG19", "ResNet50"]

    super_deep_models = ["DenseNet121"]
    
    #for model_name in small_models:
    #        print(model_name)
    #        ex_fp32 = evalSession(model_name=model_name, generated_inputs=10000) # model_float_ops must be researched
    #        ex_fp32.plot_results(batch_sizes, reps=10, verbose=True, filename=model_name)
    #        del ex_fp32

    # for model_name in large_models:
    #    print(model_name)
    #    ex_fp32 = evalSession(model_name=model_name, generated_inputs=100) # model_float_ops must be researched
    #    ex_fp32.plot_results(batch_sizes, reps=10, verbose=True, filename=model_name)
    #    del ex_fp32

    for model_name in super_deep_models:
        print(model_name)
        ex_fp32 = evalSession(model_name=model_name, generated_inputs=1) # model_float_ops must be researched
