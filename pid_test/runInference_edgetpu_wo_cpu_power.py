...
class evalSession():
    ...
    def __init__(self, tflite_model_path, input_file_folder, model_float_ops, session_name="evalSession", generated_inputs=1e4):
        # Set the path for the .xml and .bin files for the model #
        # as well as the input file directory #

        self.model_name = os.path.basename(tflite_model_path)
        self.model_path = tflite_model_path + ".tflite"
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            interpreter = tflite.Interpreter(model_path='mo_tensorflow/pidTest_unquantized.tflite')
        elif device == 'TPU':
            interpreter = tflite.Interpreter(model_path='mo_tensorflow/pidTest_edgetpu.tflite', 
        ...
        interpreter.resize_tensor_input(input_details[0]['index'], [batch_size, 16, 32, 1])
        interpreter.resize_tensor_input(input_details[1]['index'], [batch_size, 16, 32, 1])

        interpreter.allocate_tensors()

        stop_load_time = time.time()

        input_data = np.array([np.load(self.input_file_folder + "convertedInput_" + str(i) + ".npy").reshape((2,16,32,1)) for i in range(self.generated_inputs)])
        ...
        for batch in input_data:
            ...
            if (iter+1)*batch_size > self.generated_inputs:
                for _ in range((iter+1)*batch_size - self.generated_inputs):
                    batch = np.append(batch, [np.zeros((2,16,32,1), dtype=np.float32)], axis=0)

            # Setting the input
            interpreter.set_tensor(input_details[0]['index'], batch[:,0]) # data_yx
            interpreter.set_tensor(input_details[1]['index'], batch[:,1]) # data_yz
    ...
    def plot_results(self, batch_sizes, reps=1, pars=['latency', 'fps', 'flops'], verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_power/" + filename + "/"):
            os.mkdir("output_files_power/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_power/" + filename + "/" + filename + ".csv")
        ...
        fig.savefig("output_files_power/" + filename + "/" + filename + ".png")

if __name__=="__main__":
    
    batch_sizes = [1]
    
    ex_fp32 = evalSession(tflite_model_path="mo_tensorflow/pidTest_edgetpu", input_file_folder="inputs/numpy_fp32", model_float_ops=651083000, generated_inputs=10000)
    ex_fp32.plot_results(batch_sizes, reps=10, pars=['latency', 'fps', 'accuracy_degr'], verbose=True)
    del ex_fp32
