...
class evalSession():
    ...
    def __init__(self, model_name, input_file_folder, model_float_ops, session_name="evalSession", generated_inputs=1e4):
    ...
    def plot_results(self, batch_sizes, reps=1, pars=['latency', 'fps', 'flops'], verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_tpu/" + filename + "/"):
            os.mkdir("output_files_tpu/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_tpu/" + filename + "/" + filename + ".csv")
        ...
        fig.savefig("output_files_tpu/" + filename + "/" + filename + ".png")

if __name__=="__main__":
    
    batch_sizes = [1]

    sparsities = ["0.75", "0.5", "0.25", "0.0"]
    
    model_names = [ "pid_sparsity_" + spar for spar in sparsities ]


    for model_name in model_names:
        ex_fp32 = evalSession(model_name=model_name, input_file_folder="inputs/numpy_fp32/samples", model_float_ops=651083000, generated_inputs=10000)
        ex_fp32.plot_results(batch_sizes, reps=10, verbose=True)
        del ex_fp32
