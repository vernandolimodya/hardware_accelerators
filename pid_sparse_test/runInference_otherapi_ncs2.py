...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            self.net = self.ie_core.read_network(model="vpu_models/" + self.model_name + "/ir_model.xml", weights="vpu_models/" + self.model_name + "/ir_model.bin")
        elif device == 'MYRIAD':
            self.net = self.ie_core.read_network(model="vpu_models/" + self.model_name + "/ir_model_compressed.xml", weights="vpu_models/" + self.model_name + "/ir_model_compressed.bin")
    ...
    def export_results(self, batch_sizes, reps=1, pars=['latency', 'fps', 'flops'], verbose=False, filename=None):
        ...
        df_layer_data.to_csv("output_files_vpu/" + filename + "/" + filename + "_layer_perf.csv")
...
if __name__=="__main__":
    ...
    for model_name in model_names:
        ...
        ex_fp32.export_results(batch_sizes, reps=10, verbose=True)
        del ex_fp32