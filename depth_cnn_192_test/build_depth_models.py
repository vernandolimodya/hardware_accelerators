...
input_size = 32
depths = [i+1 for i in np.arange(1,17,1)]
...
for depth in depths:
  ...
  model_name = "depth_conv2d_" + str(depth)
  ...
  if not os.path.isdir("vpu_models/depth_conv2d_" + str(depth) + "/"):
    os.mkdir("vpu_models/depth_conv2d_" + str(depth) + "/")

  model.save("vpu_models/depth_conv2d_" + str(depth) + "/saved_model/")

  # save in frozen graph format #
  imported = tf.saved_model.load("vpu_models/depth_conv2d_" + str(depth) + "/saved_model/")
  ...
  tf.io.write_graph(graph_def, "vpu_models/depth_conv2d_" + str(depth) + "/", 'frozen_graph.pb', as_text=False)

  op_stats(model).to_csv("vpu_models/depth_conv2d_" + str(depth) + "/op_stat.csv")
