...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            interpreter = tflite.Interpreter(model_path='tpu_models_width/' + self.model_name + ".tflite")
        elif device == 'TPU':
            interpreter = tflite.Interpreter(model_path='tpu_models_width/' + self.model_name + '_edgetpu.tflite', 
    ...
    def plot_results(self, batch_sizes, reps=1, pars=['latency', 'fps', 'flops'], verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_tpu_width/"):
            os.mkdir("output_files_tpu_width/")

        if not os.path.exists("output_files_tpu_width/" + filename + "/"):
            os.mkdir("output_files_tpu_width/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_tpu_width/" + filename + "/" + filename + ".csv")
...
if __name__=="__main__":
    ...
    for i in [2**i for i in range(18)]:
        for j in [1]:
