...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        dir_name = 'vpu_models_ext/' + self.model_name + "/"
    ...
    def plot_results(self, batch_sizes, reps=1, verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_vpu_ext/"):
            os.mkdir("output_files_vpu_ext/")

        if not os.path.exists("output_files_vpu_ext/" + filename + "/"):
            os.mkdir("output_files_vpu_ext/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_vpu_ext/" + filename + "/" + filename + ".csv")
...
if __name__=="__main__":
    ...
    for i in [int(2**i) for i in np.arange(13, 16)]:
        for j in [int(2**i) for i in np.arange(0, 11)]:
