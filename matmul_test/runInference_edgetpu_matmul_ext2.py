...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            interpreter = tflite.Interpreter(model_path='tpu_models_ext2/' + self.model_name + ".tflite")
        elif device == 'TPU':
            interpreter = tflite.Interpreter(model_path='tpu_models_ext2/' + self.model_name + '_edgetpu.tflite', 
    ...
    def plot_results(self, batch_sizes, reps=1, pars=['latency', 'fps', 'flops'], verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_tpu_ext2/"):
            os.mkdir("output_files_tpu_ext2/")

        if not os.path.exists("output_files_tpu_ext2/" + filename + "/"):
            os.mkdir("output_files_tpu_ext2/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_tpu_ext2/" + filename + "/" + filename + ".csv")
...
if __name__=="__main__":
    ...
    for i in [int(2**i) for i in np.arange(0, 11)]:
        for j in [int(2**i) for i in np.arange(13, 16)]:
