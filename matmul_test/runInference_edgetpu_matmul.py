...
class evalSession():
    ...
    def __init__(self, model_name, session_name="evalSession", generated_inputs=1e4):
        ...
        self.model_float_ops = self.input_size*self.output_size*2 + self.output_size
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        if device == 'CPU':
            interpreter = tflite.Interpreter(model_path='tpu_models_pid/' + self.model_name + ".tflite")
        elif device == 'TPU':
            interpreter = tflite.Interpreter(model_path='tpu_models_pid/' + self.model_name + '_edgetpu.tflite', 
    ...
    def plot_results(self, batch_sizes, reps=1, pars=['latency', 'fps', 'flops'], verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_tpu_pid/"):
            os.mkdir("output_files_tpu_pid/")

        if not os.path.exists("output_files_tpu_pid/" + filename + "/"):
            os.mkdir("output_files_tpu_pid/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_tpu_pid/" + filename + "/" + filename + ".csv")
...
if __name__=="__main__":
    ...
    for i in [1024, 32768, 131072, 65536, 53760, 26880, 107520, 66560, 33280, 16896, 8448, 5120, 256, 128, 26]:
        for j in [26]:
            model_names.append("matmul_" + str(i) + "_" + str(j))

    # model_names.append("matmul_8192_8192")
