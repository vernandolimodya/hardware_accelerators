...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        dir_name = 'vpu_models_pid/' + self.model_name + "/"
        ...
        input_data = np.array([np.random.randint(-128, 127, size=(self.input_size,)).astype('float16') for z in range(self.generated_inputs)])
    ...
    def plot_results(self, batch_sizes, reps=1, verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_vpu_pid/"):
            os.mkdir("output_files_vpu_pid/")

        if not os.path.exists("output_files_vpu_pid/" + filename + "/"):
            os.mkdir("output_files_vpu_pid/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_vpu_pid/" + filename + "/" + filename + ".csv")
