...
class evalSession():
    ...
    def inference(self, batch_size=1, device='CPU', verbose=False):
        ...
        dir_name = 'vpu_models_ext2/' + self.model_name + "/"
    ...
    def plot_results(self, batch_sizes, reps=1, verbose=False, filename=None):
        ...
        if not os.path.exists("output_files_vpu_ext2/"):
            os.mkdir("output_files_vpu_ext2/")

        if not os.path.exists("output_files_vpu_ext2/" + filename + "/"):
            os.mkdir("output_files_vpu_ext2/" + filename + "/")

        df_exp_data = self.experiment(batch_sizes=batch_sizes, reps=reps, verbose=verbose)
        df_exp_data.to_csv("output_files_vpu_ext2/" + filename + "/" + filename + ".csv")
